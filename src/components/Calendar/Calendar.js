...
export class Calendar extends Component {
  ...
  componentDidMount() {
    ...
    // 初始化状态
    if (year && month && day) {
      const date_num_array = this._initMonthDayNumber(year);


      const first_day = H.weekOfMonth(new Date(year, month - 1));
  ...
  /**
     ...
     */
  componentWillMount() {
    const date_num_array = this._initMonthDayNumber(this.state.current_year);


    const first_day = H.weekOfMonth();
  ...
  /**
     ...
     */
  selectDate(s_day) {
    const { select_year, select_month } = this.state;
    this.setState({
  ...
  previousMonth() {
    let {
      ...
      select_year, select_month, select_day, date_num_array, first_day,
    } = this.state;
  ...
  /**
     ...
     */
  render() {
    const { row_number, col_number, tags } = this.props;
    const {
      current_year, current_month, current_day,
      select_year, select_month, select_day,
      history_year, history_month, history_day,
      date_num_array, first_day,
    } = this.state;

    const month_day = date_num_array[select_month];


    const n_day = row_number * col_number - first_day - month_day;
    ...
    for (let i = 0; i < first_day; i++) {
      const previous_link = (
        <li className="item-gray" key={'previous' + i}>
    ...
    for (let i = 0; i < month_day; i++) {
      ...
      const current_link = (
        <li className={currentClassName} key={'current' + i}>
    ...
    for (let i = 0; i < n_day; i++) {
      const next_link = (
        <li className="item-gray" key={'next' + i}>
    ...
    if (total_days.length > 0) {
      for (let i = 0; i < row_number; i++) {
        ...
        const start_index = i * col_number;


        const end_index = (i + 1) * col_number;
      ...
      <div className="calendar">
        ...
        <div className="calendar-body">
          ...
          <div className="c-body-content">
            {
              ul_list.map((u, index) => (<ul key={'ul' + index} className="content-row">{u}</ul>))
            }
          </div>
        </div>
      </div>
