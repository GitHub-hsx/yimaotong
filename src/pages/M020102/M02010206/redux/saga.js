 ...
 * @LastEditTime: 2019-08-10 11:14:32
...
export function* fetchAccountOverView() {
  while (true) {
    ...
    const requestUrl = ApiConfig.HOST_NAME_BASE_URL + '/business/purch/purchOffer';
    let data;
    if (action.type === BUSINESS_SUPPLY) {
      try {
        ...
        if (data.status == '1') {
          ...
          yield put(businessSupplyError('您的报价信息已提交，请等待买家确认'));
        } else {
